...
```
    function GrumpsEstimation.θcallback( ::Val{ :myid }, statevec, e, d, o, oldx, repeatx, solution ) 
...
This will print "hi" on every $θ$ iteration.  The first argument of `GrumpsEstimation.θcallback` specifies which id this callback refers to (in case there is more than one), `statevec` is the state vector of the `Optim` package (see the documentation of that package for details), `oldx` is the $θ$-vector value of the previous iteration, and `repeatx` is a single element vector that indicates how often the same value of the parameter vector has been repeated.  Messing with the values of the arguments is not recommended.  The `GrumpsEstimation.δcallback` function has the same syntax but lacks the `solution` argument.

If the `id` variable is set but no user callbacks are defined then Grumps will only execute the default callbacks.

!!! note "Do not overuse GrumpsEstimation.δcallback"
...
```
    function GrumpsEstimation.CreateInteractions( ::Val{ :myid }, dfc, dfp, v, T )
        ...
        ...
        ...
    return Z
