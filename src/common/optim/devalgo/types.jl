...
struct NewterMethod{T<:Flt}
    ...
    function NewterMethod( ρl :: T2, ρu :: T2, Δh :: T2, iΔ :: T2, η :: T2 ) where {T2<:Flt}
...
mutable struct NewterState{T<:Flt}
    ...
    function NewterState( x_previous :: VVector{T2}, x :: VVector{T2}, f_x_previous :: T2, s :: VVector{T2},
...
struct NewterOptions{T<:Flt}
    ...
    function NewterOptions( T = Float64; initial_Δ =1.0, Δhat = 100.0, η = 0.1, ρ_lower = 0.25, ρ_upper = 0.75, 
...
mutable struct Newter{T<:Flt}
    ...
    function Newter( f :: Function, xstart :: VVector{T2}, data :: Vector{GrumpsMarketData{T2}}, K :: VMatrix{T2} ) where {T2<:Flt}
...
struct NewterTrace1{T<:Flt} 
    iteration   :: Int
    f           :: T
    x           :: VVector{T}
    gnorm       :: T
    dt          :: Dict
    function NewterTrace1( iteration :: Int, f::T2, x :: VVector{T2}, gnorm :: T2, dt :: Dict ) where {T2<:Flt}
...
mutable struct NewterTrace{T<:Flt} 
    tr          :: Vector{ NewterTrace1{T} }
    starttime   :: Float64

    function NewterTrace( T )
        new{T}( VVector{ NewterTrace1{T} }(undef, 0), time() )
    end
end
